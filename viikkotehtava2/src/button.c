...
{
	printk("Button 2 YELLOW pressed\n");
    if (tilakone != 6) {
        saved_state = tilakone; 
        tilakone = 6;           
    } else {
...
{
	printk("Button 4 (Nappi 5) YELLOW BLINK sequence pressed\n");
    if (tilakone != 8) {
        saved_state = tilakone;
        tilakone = 8; 
    } else {
...
int init_button() {
	...
	tsekkaa_nappi(&button_1, &button_1_data, button_1_handler);
	tsekkaa_nappi(&button_2, &button_2_data, button_2_handler);
	tsekkaa_nappi(&button_3, &button_3_data, button_3_handler);
	tsekkaa_nappi(&button_4, &button_4_data, button_4_handler);
	return 0;
}